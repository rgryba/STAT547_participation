...
A few diff ways to write the syntax.
```{r}
...
IF test passes there is no return; expect_identical there is error because of floating point issue.
```{r error=TRUE}
...
#Notes:
Use if..stop() to through errors within a function.
Use testthat after a function is written to test the actual function. 