...
```{r}
jsonlite::fromJSON(
  "http://www.omdbapi.com/?t=Jurassic+Park&apikey=6a4112f0"
)
```
...
```{r}
site$status_code
#site$content #not useful - specific to httr package - storing raw content in bites
...
names(site)
status_code(site)
content(site) #gives a list this time because github has an api
```
